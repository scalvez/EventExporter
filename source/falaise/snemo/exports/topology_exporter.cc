...
namespace snemo {
    ...
    namespace exports {
      ...
      bool topology_exporter::is_initialized () const
      {
        return _initialized_;
      }

      bool topology_exporter::has_geom_manager () const
      {
        return _geom_manager_ != 0;
      }

      const geomtools::manager & topology_exporter::get_geom_manager () const
      {
        return *_geom_manager_;
      }

      void topology_exporter::set_geom_manager (const geomtools::manager & gmgr_)
      {
      ...
      uint32_t topology_exporter::get_export_flags () const
      {
        return _export_flags_;
      }

      void topology_exporter::unset_exported (unsigned int export_bit_)
      {
        _export_flags_ &= ~export_bit_;
        return;
      }

      void topology_exporter::set_exported (unsigned int export_bit_)
      {
        _export_flags_ |= export_bit_;
        return;
      }

      bool topology_exporter::is_exported (unsigned int export_bit_) const
      {
        return (_export_flags_ & export_bit_);
      }

      int topology_exporter::get_topic_export_level(const std::string & topic_label_) const
      {
        if (topic_label_ == "CAT")
          {
      ...
      void topology_exporter::initialize (const datatools::properties & setup_)
      {
        ...
        if (setup_.has_flag ("export.topology_2e"))
          {
            set_exported (sre::topology_exporter::EXPORT_TOPOLOGY_2E);
          }
      ...
      void topology_exporter::reset ()
      {
        DT_THROW_IF (! is_initialized (), std::logic_error,  "Topology exporter is not initialized ! ");
        _initialized_ = false;
        _init_defaults ();
        return;
      }

      void topology_exporter::_set_default_bank_labels ()
      {
        _bank_labels_.clear ();
      ...
      void topology_exporter::_init_defaults ()
      {
                                  ...
                                  sre::export_topology & et_)
      {
        ...
        et_.clear_data ();
        if (is_exported (sre::topology_exporter::EXPORT_TOPOLOGY_2E))
          {
      ...
      topology_exporter::get_bank_labels () const
      {
      ...
      int topology_exporter::_export_topology_2e (const datatools::things & er_,
                                                  sre::export_topology & et_)
      {
        const std::string & td_label = _bank_labels_[sdm::data_info::TOPOLOGY_DATA_LABEL];
        ...
        et_.event_header.run_number   = EH.get_id ().get_run_number ();
      ...
      int topology_exporter::_export_true_particles (const datatools::things & er_,
      ...
      int topology_exporter::_export_true_step_hits (const datatools::things & er_,
                                                  sre::export_event & ee_)
      {
        const std::string & sd_label = _bank_labels_ [sdm::data_info::SIMULATED_DATA_LABEL];
        if (! DATATOOLS_THINGS_CHECK_BANK(er_, sd_label, mctools::simulated_data))
      ...
      int topology_exporter::_export_true_hits (const datatools::things & er_,
                                             sre::export_event & ee_)
      {
        const std::string & sd_label = _bank_labels_[sdm::data_info::SIMULATED_DATA_LABEL];
        if (! DATATOOLS_THINGS_CHECK_BANK(er_, sd_label, mctools::simulated_data))
      ...
      int topology_exporter::_export_calib_calorimeter_hits (const datatools::things & er_,
                                                          sre::export_event & ee_)
      {
      ...
      int event_exporter::_export_particle_tracks (const datatools::things & er_,
                                                   sre::export_event & ee_)
      {
        const std::string & ptd_label = _bank_labels_[sdm::data_info::default_particle_track_data_label()];
        if (! DATATOOLS_THINGS_CHECK_BANK(er_, ptd_label, sdm::particle_track_data))
          {
            DT_THROW_IF (true, std::logic_error, "Missing particle track data to be processed !");
          }
        DATATOOLS_THINGS_CONST_BANK(er_, ptd_label, sdm::particle_track_data, PTD);
